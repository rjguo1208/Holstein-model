...
int main(int argc,char** argv){try{
    ...
    const std::vector<std::string> allowed={"t","omega","g","U","mu","origin","tau-max","max-arcs","max-hop-pairs","bins","blocks","steps-per-block","warmup","thin","radius","seed","out"};
    ...
    sampler.reset();
    std::ofstream csv(out/"blocks.csv"),prof(out/"profiles.csv");csv<<std::setprecision(17);prof<<std::setprecision(17);
    csv<<"block,reference,samples";
    for(auto name:{"count","energy","zero_ph","nph","p0","r2"})for(unsigned b=0;b<bins;++b)csv<<","<<name<<"_"<<b;
    csv<<"\n";
    // Three projection windows [T/4,T/2], [T/2,3T/4], [3T/4,T].
    prof<<"block";const unsigned sites=2*radius+2;
    for(unsigned w=0;w<3;++w)for(auto name:{"p","z"})for(unsigned x=0;x<sites;++x)prof<<","<<name<<"_"<<w<<"_"<<x;
    prof<<"\n";
    for(unsigned block=0;block<blocks;++block){
        std::vector<std::vector<double>> fields(6,std::vector<double>(bins,0));
        std::vector<double> profiles(3*2*sites,0),probability(bins),energy(bins);
        ...
        for(std::uint64_t step=0;step<steps;++step){
            ...
            for(unsigned b=0;b<bins;++b){
                const double p=probability[b];fields[0][b]+=p;fields[1][b]+=energy[b];fields[2][b]+=p*(a.nph_mid==0);fields[3][b]+=p*a.nph_mid;fields[4][b]+=p*(a.midpoint==0);fields[5][b]+=p*a.midpoint*a.midpoint;
            }
        ...
        prof<<block;for(double x:profiles)prof<<","<<x;prof<<"\n";prof.flush();
    }
