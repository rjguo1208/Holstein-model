...
from refinement import exact_moments
...
def solve(tau, green, cov, p, alpha, size=281, upper=10., tau_limit=8.,
    ...
    for iteration in range(max_iterations):
        ...
        step = long_solve(hessian / np.outer(scale, scale), -grad / scale) / scale
        slope = grad @ step
        if not np.isfinite(step).all() or slope >= 0:
            raise ValueError('Newton direction is not a finite descent direction')
...
def analytic_checks():
    p = dict(t=1., omega=1., g=np.sqrt(.5), k=.7, tau_max=6., bins=24, mu=-3.)
    en = np.linspace(-2.5, 5., 121)
    tau = (np.arange(p['bins']) + .5) * p['tau_max'] / p['bins']
    weights = np.exp(-.5 * (en - .2) ** 2)
    weights /= weights.sum()
    moments = np.array([weights.sum(), en @ weights, (en ** 2) @ weights])
    green = bin_kernel(tau, p['tau_max'] / p['bins'], en, p['mu']) @ weights
    cov = np.diag((green * .01) ** 2)
    records = []
    # This distribution minimizes entropy at its moments and fits data exactly,
    # so it is an analytic optimum for every positive alpha.
    for alpha in [.1, 10., 1000.]:
        fit = solve(tau, green, cov, p, alpha, energies=en, target_moments=moments)
        error = float(max(abs(fit['weights'] - weights)))
        assert fit['converged'] and error < 2e-7, (alpha, error, fit['dual_gradient'])
        records.append(dict(alpha=alpha, maximum_weight_error=error,
                            dual_gradient=fit['dual_gradient'], moment_error=float(max(abs(fit['moment_residuals'])))))
    return records
